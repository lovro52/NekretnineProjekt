          ...
          <v-card class="card-border" width="600px" outlined>
            ...
            <v-card-text class="card-text-border">
              <v-form v-model="valid">
                <v-text-field v-model="firstName" dense label="Name" clearble type="text" outlined></v-text-field>
                <v-text-field v-model="lastName" dense label="Last Name" clearble type="text" outlined></v-text-field>
                <v-text-field v-model="email" dense label="Email" clearble type="text" outlined></v-text-field>
                <v-text-field v-model="newPassword" dense label="Password" clearble :append-icon="showIcon ? 'mdi-eye' : 'mdi-eye-off'
                  " :rules="[rules.required, rules.min]" :type="showIcon ? 'text' : 'password'" outlined></v-text-field>
              </v-form>
            </v-card-text>
            <v-card-actions class="card-actions">
              <v-btn :disabled="isChangePassButtonDisabled ||
                newPassword == null ||
                !valid
                " outlined @click="changePassword">
                CHANGE PASSWORD
              </v-btn>
